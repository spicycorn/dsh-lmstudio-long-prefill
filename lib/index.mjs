// dsh-local-model-long-prefill v1.1
//
// Root cause of the repeated local-provider timeouts (three layers):
//
...
//    300000 ms). The timer re-arms on every chunk, so during long prefill it
//    measures from request start until the first token; if nothing arrives in
//    that window the watchdog aborts with "pi-ai stream idle timeout after <ms>ms".
//
// 3. OpenAI SDK per-request timeout (NEW layer found in v1.1): when a provider
//    has no explicit `timeoutMs`, dsh-llm-pi-ai forwards none and the openai
//    client falls back to its DEFAULT_TIMEOUT of 600000 ms — fetchWithTimeout()
//    aborts fetch() exactly at that deadline for response HEADERS. Every long
//    prefill therefore dies at precisely +10 minutes with the SDK's
//    APIConnectionTimeoutError, surfaced by DSH as a retryable failure
//    "Request timed out." (observed 25+ times in one session: each attempt
//    failed exactly ~600 s after step start; retries landed ~470 ms later).
//
// This plugin fixes ALL THREE on every load:
//   1. Self-heals the OpenAI SDK shims patch (node:http, no timeout).
//   2. Automatically rewrites settings.yaml for every local provider (baseURL
//      pointing to 127.0.0.1, localhost, or a LAN address): raises BOTH
//      `timeoutMs` (OpenAI SDK per-request header deadline; NEW in v1.1) and
//      `streamIdleTimeoutMs` (pi-ai idle watchdog) — no manual config editing.
...
const HEADER_COMMENT = `// [${MARKER}] v1.1 getDefaultFetch routed through node:http for local
...
// overwrites the openai package is healed on the next DSH start. The same
// plugin also manages timeoutMs/streamIdleTimeoutMs in settings.yaml for local providers.`;
...
/* -------------------------------------------------------------------------- */
/* settings.yaml auto-rewrite: raise BOTH timeout knobs for local providers.  */
/*                                                                            */
...
/*                                                                            */
/* NEW in v1.1. The OpenAI SDK per-request timeout (client default 600s) was    */
/* a third layer that fired BEFORE both of the above for long prefills,        */
/* producing "Request timed out." at exactly +10 minutes on every attempt      */
/* (25+ observed in one session). dsh-llm-pi-ai forwards an explicit           */
/* provider `timeoutMs` to the SDK request-level timeout, overriding that       */
/* default — so this plugin now writes BOTH fields for local providers:        */
/*   - timeoutMs            (OpenAI SDK header deadline; was 600s by default)   */
/*   - streamIdleTimeoutMs  (pi-ai idle watchdog; raised from the v1.0 target,  */
/*                           because session evidence showed prefills that      */
/*                           outlast even a 30-minute window: "pi-ai stream     */
/*                           idle timeout after 1800000ms")                     */
/* Both use one shared budget so no layer can fire before another. The         */
/* watchdog re-arms on every chunk, so the large value cannot mask a            */
/* slow-but-progressing stream; it only bounds truly silent requests.          */
/* -------------------------------------------------------------------------- */

// One timeout budget for local/LAN providers (2 hours). Session evidence:
// long prefills exceeded even 30 minutes ("pi-ai stream idle timeout after
// 1800000ms"), and the SDK default of 600s killed every attempt at exactly
// +10 min. A hung local server now fails within ~2h instead of never failing;
// lower this constant if you want faster failure detection on your hardware.
const DEFAULT_LOCAL_TIMEOUT_MS = 7_200_000; // 2 hours

/** The two dsh-llm-pi-ai provider fields managed by the settings rewrite, in stable order. */
const MANAGED_PROVIDER_FIELDS = ["timeoutMs", "streamIdleTimeoutMs"];
...
/**
 * Rewrite settings.yaml to ensure every local provider has BOTH managed fields
 * (timeoutMs, streamIdleTimeoutMs) at or above DEFAULT_LOCAL_TIMEOUT_MS.
 */
function patchSettingsYaml(log) {
  ...
  if (yaml) {
    ...
    for (const [name, cfg] of Object.entries(providers)) {
      if (!isLocalProviderConfig(cfg) || !cfg || typeof cfg !== "object") continue;
      for (const field of MANAGED_PROVIDER_FIELDS) {
        const currentRaw = cfg[field];
        const currentNum = Number(currentRaw);
        // Raise unset, non-numeric, or below-target values to the shared budget.
        if (!Number.isFinite(currentNum) || currentNum < DEFAULT_LOCAL_TIMEOUT_MS) {
          cfg[field] = DEFAULT_LOCAL_TIMEOUT_MS;
          changed = true;
          log(`settings.yaml: provider "${name}" ${field} ${currentRaw === undefined ? "(unset)" : String(currentRaw)} → ${DEFAULT_LOCAL_TIMEOUT_MS}`);
        }
      }
    }
...
function patchSettingsYamlLineBased(content, settingsPath, log) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inProviders) {
      ...
      if (currentProvider) {
        ...
        if (urlMatch) currentProvider.baseURL = urlMatch[1].replace(/["',]/g, "");
        // Any indented field belongs to the block: extend its end so keys that
        // follow baseURL are still seen by the managed-field checks below.
        if (/^\s{4,}\S/.test(line)) currentProvider.endLine = i;
  ...
  for (const block of providerBlocks) {
    ...
    // Indentation for inserted keys: reuse the block's first field line, else 8.
    let indent = "        ";
    for (let i = block.startLine + 1; i <= block.endLine; i++) {
      const m = lines[i].match(/^(\s+)\S/);
      if (m) { indent = m[1]; break; }
    }
    // Manage every field independently: raise in place when below target, or
    // queue an insertion after the block's last line. Insertions for one block
    // are queued bottom-up so they land top-down in MANAGED_PROVIDER_FIELDS order.
    const missing = [];
    let anchor = block.endLine;
    for (const field of [...MANAGED_PROVIDER_FIELDS].reverse()) {
      // Match the key with an optional quoted/numeric value so a hand-edited
      // `timeoutMs: "7200000"` is recognized, not duplicated.
      const re = new RegExp(`^\\s+${field}:`);
      let foundIdx = -1;
      for (let i = block.startLine + 1; i <= anchor; i++) {
        if (re.test(lines[i])) { foundIdx = i; break; }
      }
      if (foundIdx !== -1) {
        const digits = lines[foundIdx].match(/\d+/);
        const val = digits ? parseInt(digits[0], 10) : NaN;
        // Raise in place when the value is missing, non-numeric, or below target.
        if (!Number.isFinite(val) || val < DEFAULT_LOCAL_TIMEOUT_MS) {
          const leadMatch = lines[foundIdx].match(/^(\s*)/)[1];
          lines[foundIdx] = `${leadMatch}${field}: ${DEFAULT_LOCAL_TIMEOUT_MS}`;
          changed = true;
          log(`settings.yaml: provider "${block.name}" ${field} ${Number.isFinite(val) ? val : "(invalid/unset)"} → ${DEFAULT_LOCAL_TIMEOUT_MS}`);
        }
      } else {
        missing.push(field); // inserted below, after `anchor` (grows downward)
      }
    }
    for (const field of [...missing].reverse()) {
      insertions.push({ afterLine: anchor, line: `${indent}${field}: ${DEFAULT_LOCAL_TIMEOUT_MS}` });
      changed = true;
      log(`settings.yaml: provider "${block.name}" ${field} (unset) → ${DEFAULT_LOCAL_TIMEOUT_MS}`);
    }
  }

  // Apply insertions bottom-up so earlier anchors stay valid. Same-anchor lines:
  // the LAST one spliced at a position lands closest to its anchor, so process
  // them in ascending field rank — yielding top-down MANAGED_PROVIDER_FIELDS order.
  const fieldRank = (ins) => {
    for (let k = 0; k < MANAGED_PROVIDER_FIELDS.length; k++) if (ins.line.includes(`${MANAGED_PROVIDER_FIELDS[k]}:`)) return k;
    return -1;
  };
  insertions.sort((a, b) => b.afterLine - a.afterLine || fieldRank(b) - fieldRank(a));
...
export function apply(ctx, input = {}) {
  ...
  for (const shim of shims) {
    ...
    if (result === "applied") debug(`dsh-local-model-long-prefill v1.1: patched ${shim}`);
  }
  if (results.length === 0) {
    debug("dsh-local-model-long-prefill v1.1: no openai shims found; global fetch fallback is the only protection");
  } else {
    for (const r of results) debug(`dsh-local-model-long-prefill v1.1: ${r}`);
  }

  // 2) Auto-rewrite settings.yaml: raise streamIdleTimeoutMs for local providers.
  const settingsResult = patchSettingsYaml(debug);
  debug(`dsh-local-model-long-prefill v1.1: settings.yaml → ${settingsResult}`);
  ...
  ctx.effect(() => {
    ...
    debug("dsh-local-model-long-prefill v1.1: global fetch fallback restored (file patches retained)");
